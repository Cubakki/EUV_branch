#检查second_calculation中orca_workdir内的计算结果是否成功，删去不成功的计算目录并同步worked
...
if __name__=="__main__":
    ...
    for stru in os.listdir(Workdir):
        ...
        try:
            if (os.path.exists(stru_dir + "/ng/output.log") and os.path.exists(stru_dir + "/ps/output.log")):
                with open(stru_dir + "/ng/output.log", "r") as f:
                    if "****ORCA TERMINATED NORMALLY****" in f.readlines()[-2]:
                        ...
                        continue
                with open(stru_dir + "/ps/output.log", "r") as f:
                    if "****ORCA TERMINATED NORMALLY****" in f.readlines()[-2]:
                        pass
                    else:
                        print(f"{stru} was not correctly finished")
